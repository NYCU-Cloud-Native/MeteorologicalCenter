...
export class EarthquakeController {
  ...
  async Run(): Promise<void> {
    ...
    const earthquake = await this.earthquakeService.createEarthquake(
      ...
      data.records.Earthquake[0].EarthquakeInfo.EarthquakeMagnitude.MagnitudeValue,
    )

    console.log(data.records.Earthquake[0].EarthquakeInfo.Epicenter.Location);
    console.log(data.records.Earthquake[0].EarthquakeInfo.EarthquakeMagnitude.MagnitudeValue)
